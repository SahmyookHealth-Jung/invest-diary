...
export default function HoldingsPage() {
  ...
  const handleSell = async () => {
    ...
    const { error: tradeErr } = await supabase.from("trades").insert({
      ...
      notes: sellTarget.notes
        ? `[보유→청산] ${sellTarget.notes}`
        : "[보유→청산]",
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("이 보유 종목을 삭제할까요? (매매일지에 기록되지 않습니다)"))
      return;
  ...
  const totalInvested = holdings.reduce(
    (sum, h) => sum + h.entry_price * h.quantity,
    0
  );
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, stop_price: e.target.value })
                }
                placeholder="선택"
        ...
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-t-2xl bg-zinc-900 p-5 space-y-4 safe-area-bottom">
            ...
            <div className="rounded-lg bg-zinc-800 p-3 text-sm text-zinc-400">
              <p>
                진입가: {formatCurrency(sellTarget.entry_price)} × {sellTarget.quantity}주
              </p>
              <p>
                투자금: {formatCurrency(sellTarget.entry_price * sellTarget.quantity)}
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium text-zinc-400 mb-1">
            ...
            return (
              <li
                ...
                className={`rounded-xl bg-zinc-900 p-4 ${
                  isLongHold ? "border border-amber-500/40" : ""
                }`}
              >
